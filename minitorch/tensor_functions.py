...
class Sigmoid(Function):
    ...
    def backward(ctx: Context, grad_output: Tensor) -> Tensor:
        ...
        one = minitorch.Tensor.make([1.0]*sigmoid_out.size, sigmoid_out.shape, backend=sigmoid_out.backend)
        grad = sigmoid_out.f.mul_zip(
            sigmoid_out.f.add_zip(one, sigmoid_out.f.neg_map(sigmoid_out)),
            sigmoid_out
        )
        return grad_output.f.mul_zip(grad_output, grad)
...
class ReLU(Function):
    ...
    def backward(ctx: Context, grad_output: Tensor) -> Tensor:
        # TODO: Implement for Task 2.4.
        (relu_out,) = ctx.saved_values
        return grad_output.f.relu_back_zip(relu_out, grad_output)
...
class Permute(Function):
    ...
    def forward(ctx: Context, a: Tensor, order: Tensor) -> Tensor:
        # TODO: Implement for Task 2.3.
        order_tuple = tuple(int(order[i].item()) for i in range(order.shape[0]))
    ...
    def backward(ctx: Context, grad_output: Tensor) -> Tuple[Tensor, float]:
        ...
        for i, o in enumerate(order_tuple):
            inverse_order[o] = i
